-- =====================================================
-- MIGRATION: Google Sheets Integration
-- =====================================================
-- This file contains the complete SQL migration for Google Sheets integration
-- Copy and paste this entire content into the Supabase SQL Editor
-- =====================================================

-- Drop existing tables if they exist (for re-running the migration)
DROP TABLE IF EXISTS google_sheets_sync_history CASCADE;
DROP TABLE IF EXISTS google_sheets_integrations CASCADE;
...
CREATE INDEX idx_google_sheets_integrations_org_module
  ON google_sheets_integrations(organization_id, module_name);

CREATE INDEX idx_google_sheets_integrations_active
  ON google_sheets_integrations(organization_id, is_active);

-- Create sync history table
CREATE TABLE google_sheets_sync_history (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE INDEX idx_google_sheets_sync_history_integration
